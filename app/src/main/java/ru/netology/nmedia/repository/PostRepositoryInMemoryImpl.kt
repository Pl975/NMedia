...
class PostRepositoryInMemoryImpl : PostRepository {
    ...
    private var posts = listOf(
        ...
        Post(
            ...
            likedByMe = false,
            sharedByMe = false,
            video = PostVideo("Весна - открытый фильм","https://www.youtube.com/watch?v=WhWc3b3KhnY")
        ),
    ).reversed()
    ...
    override fun shareById(id: Long) {
        posts = posts.map {
            if (it.id != id) it else it.copy(sharedByMe = !it.sharedByMe, shareCount = it.shareCount + 1)
        }
        data.value = posts
    }
